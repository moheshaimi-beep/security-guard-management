...
async function createTestAgent() {
  try {
    ...
    // Créer l'agent de test SANS hook pour éviter le double chiffrement
    const testAgent = await User.create({
      ...
      role: 'agent',
      status: 'active',
      facialVector: facialVectorJSON, // Déjà encrypté comme string JSON
    ...
    console.log('Agent de test créé avec succès! Vérification de la base de données...');

    // Vérifier immédiatement l'agent créé
    const verifyAgent = await User.findOne({ where: { employeeId: 'TEST001' } });
    console.log('Agent en DB après création:');
    console.log('  Type de facialVector:', typeof verifyAgent.facialVector);
    console.log('  Longueur:', verifyAgent.facialVector?.length);
    console.log('  Préfixe (50):', verifyAgent.facialVector?.substring(0, 50));
