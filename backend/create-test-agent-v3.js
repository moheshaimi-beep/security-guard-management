...
async function createTestAgent() {
  try {
    ...
    // Créer l'agent de test avec hooks activés
    const testAgent = await User.create({
      ...
      role: 'agent',
      status: 'active',
      facialVector: testFacialVector, // Array de floats - le hook d'encryption va le traiter
      facialVectorUpdatedAt: new Date(),
      supervisorId: null,
      createdByType: 'admin',
      createdByUserId: null,
    }, { hooks: true }); // Hooks activés pour l'encryption
    ...
    console.log('FacialVector après création (type, longueur):',
      ...
      createdAgent.facialVector?.substring(0, 50)
    );
